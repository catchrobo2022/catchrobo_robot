...
if __name__ == "__main__":
    ...
    pulley_radius = 0.002 * 54/(2*math.pi)
    radius_delta_ = 0.070 - 0.061 #穴のおおきさ - じゃがりこ底面
    velocity = 2 * math.pi * radius_delta_

    command = MyRosCmd()

    command.id = 0 # 0: x軸 1:y軸 2: z軸 3:グリッパー
    command.mode = MyRosCmd.PEG_IN_HOLE_MODE# MyRosCmd.POSITION_CTRL_MODE or MyRosCmd.VELOCITY_CTRL_MODE
    command.position = radius_delta_ /pulley_radius 
    command.velocity = velocity /pulley_radius  #math.pi#目標位置での速度
    command.mass = 0.0 # 慣性モーメント(未対応)
    command.effort = 0 #自重補償項(未対応)
    command.position_min = -1 /pulley_radius #可動域
    command.position_max = 1  /pulley_radius #可動域
    ...
    rospy.sleep(20) #このプログラムは3秒後に自動終了する. このsleepが無いとpublish前に終了してしまう


    for i in range(3):
        command_array.command_array[i].mode = MyRosCmd.POSITION_CTRL_MODE
        command_array.command_array[i].position = 0
        command_array.command_array[i].velocity =0
