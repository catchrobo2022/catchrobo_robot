...
from joy_controller_class.ps4_button_wireless import PS4Button_wireless
from joy_controller_class.ps4_button import PS4Button
    ...
    def __init__(self):
       ...
       #　position ,velocityの初期値はをここでいじる
        self.command_x = self.template.generate_ros_command(
            id=0, mode=MyRosCmd.POSITION_CTRL_MODE, robot_position=0.0, robot_end_velocity=0.0)
        self.command_y = self.template.generate_ros_command(
            id=1, mode=MyRosCmd.POSITION_CTRL_MODE, robot_position=0.0, robot_end_velocity=0.0)
        self.command_z = self.template.generate_ros_command(
            id=2, mode=MyRosCmd.POSITION_CTRL_MODE, robot_position=0.0, robot_end_velocity=0.0)
        self.command_g = self.template.generate_ros_command(
            id=3, mode=MyRosCmd.POSITION_CTRL_MODE, robot_position=0.0, robot_end_velocity=0.0)  # gripperのg
...
class Manual():
    ...
    def manualControl(self):
        ...
        if(self.button_disable == False):
            ...
            #x軸
            if(joy_a[b_num.RIGHT_LEFT] == 1 and self.axes_count[b_num.RIGHT_LEFT] == 0):
                self.posSet(0)
                self.position_var_manual[0] += cmd_tmp.robot_m2rad(
                    cmd_x.id, 0.005*self.COLOR_NUM)
                self.axes_count[b_num.RIGHT_LEFT] = 1
                print("b_left")
            elif(joy_a[b_num.RIGHT_LEFT] == -1 and self.axes_count[b_num.RIGHT_LEFT] == 0):
                self.posSet(0)
                self.position_var_manual[0] -= cmd_tmp.robot_m2rad(
                    cmd_x.id, 0.005*self.COLOR_NUM)
                self.axes_count[b_num.RIGHT_LEFT] = 1
                ...
                self.axes_count[b_num.RIGHT_LEFT] = 0

            #y軸
            if(joy_a[b_num.UP_DOWN] == 1 and self.axes_count[b_num.UP_DOWN] == 0):
                self.posSet(1)
                self.position_var_manual[1] -= cmd_tmp.robot_m2rad(
                    cmd_y.id, 0.005*self.COLOR_NUM)
                self.axes_count[b_num.UP_DOWN] = 1
                print("b_forward")
            elif(joy_a[b_num.UP_DOWN] == -1 and self.axes_count[b_num.UP_DOWN] == 0):
                self.posSet(1)
                self.position_var_manual[1] += cmd_tmp.robot_m2rad(
                    cmd_y.id, 0.005*self.COLOR_NUM)
                self.axes_count[b_num.UP_DOWN] = 1
                ...
                self.axes_count[b_num.UP_DOWN] = 0
            ...
            if(joy_b[b_num.CIRCLE] == 1 and self.button_count[b_num.CIRCLE] == 0):
                self.posSet(3)
                self.position_var_manual[3] = cmd_tmp.robot_m2rad(
                    cmd_g.id, 1.5)
                self.button_count[b_num.CIRCLE] = 1
                print("release")
                ...
                self.position_var_manual[3] = cmd_tmp.robot_m2rad(
                    cmd_g.id, 0.0)
                self.button_count[b_num.CIRCLE] = 3
                print("grab")
        ...
        # 一回目押したとき、pauseを維持
        if(joy_b[b_num.CROSS] == 1 and (self.button_count[b_num.CROSS] == 0 or self.button_count[b_num.CROSS] == 1)):
            #　本来はここにはいらないけど、安全のために一応入れとく
            self.pauseProcess()
            self.button_count[b_num.CROSS] = 1
            #print("pause start")
        elif(joy_b[b_num.CROSS] == 0 and (self.button_count[b_num.CROSS] == 1 or self.button_count[b_num.CROSS] == 2)):
            self.pauseProcess()
            self.button_count[b_num.CROSS] = 2
            print("pause")
        # 二回目押したとき、pause機能を停止
        elif(joy_b[b_num.CROSS] == 1 and self.button_count[b_num.CROSS] == 2):
            #これ以降の分岐はSQUAREの分岐の方の基本の方でposが入力される
            self.button_count[b_num.CROSS] = 3
            #print("restart start")
        elif(joy_b[b_num.CROSS] == 0 and self.button_count[b_num.CROSS] == 3):
            self.button_count[b_num.CROSS] = 0
            print("restart")
...
if __name__ == "__main__":
    rospy.init_node("manual_ps4_node", anonymous=True)
